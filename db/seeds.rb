...
# query for words in postgreSQL: select concat('INSERT INTO "words" ("word", "translation", "created_at", "updated_at", "user_id", "techno_id") VALUES (''',replace(w.word,'''', ''''''),''', E''',replace(replace(replace(w.translation,'''', ''''''), '\n', '\\n'),'\r','\\r'),''', NOW(), NOW(), (select id from users where email=''',u.mail,''' limit 1), (select t0.id from technos as t0 inner join users as u0 on t0.user_id=u0.id where t0.techno_name=''',t.technology,''' and u0.email=''',u.mail,''' limit 1));') as query from words as w inner join technos as t on w.techno_id=t.techno_id inner join users as u on t.user_id=u.user_id;